...
export function SiteContentProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => { if (!db) return; const offSettings = onSnapshot(doc(db, "siteContent", "settings"), (snapshot) => { setSettings(snapshot.exists() ? snapshot.data() as SiteSettings : null); setSettingsReady(true); }); const offMenu = onSnapshot(collection(db, "menuItems"), (snapshot) => { setMenuItems(snapshot.docs.map((item) => ({ id: item.id, ...item.data() } as MenuItem)).filter((item) => item.visible).sort(compareMenuItems)); setMenuReady(true); }); return () => { offSettings(); offMenu(); }; }, []);
